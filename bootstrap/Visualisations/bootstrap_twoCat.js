...
class bootstrap_twoCat extends visBase {
	...
	buildUpSlow(settings, currentAnimation, upto, popText, max, self, seen){
		...
		for(let sc = 0; sc < sample_circles.length; sc++){
			...
			let primary_category = id.split('-')[0];
			let secondary_category = id.split('-').slice(1, id.split('-').length - 1).join('-');
			let id_values = id.split('-');
			if(seen.includes(id)) continue;
			let primary_category_name = [...self.valueCategories][settings.sample[upto].value]
			if((settings.sample[upto].group == secondary_category) && (primary_category_name == primary_category)){
